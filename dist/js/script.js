...
(function () {
	...
	$(document).ready(function () {
		...
		$(".forms-box").on("click", ".delete-input", function () {
			...
			if (searchResult.length === 1) {
				...
				var tmp = adminInputs[searchResult[0]];
		...
		$(".forms-box").on("click", ".add-subinput", function () {
			var defaultSchemeInputs = mixins.getDefaultSchemeInputs();
			var mySection = $(this).closest("section"),
		...
		$("#preview").on("keyup mouseup", "form input[type='number']", function () {
			...
			var level = parseInt($(this).closest("[data-level]").data("level"));
			var less_than = $("form[data-level='" + (level + 1) + "'][data-if='less_than']");
			var greater_than = $("form[data-level='" + (level + 1) + "'][data-if='greater_than']");
			...
			for (var i = 0; i < less_than.length; i++) {
				var currentForm = less_than.eq(i);
				var form_value = parseInt(currentForm.data("ifValue"));
				if (form_value > value) {
					currentForm.removeClass("hidden");
				}
			}

			for (var _i = 0; _i < greater_than.length; _i++) {
				var _currentForm = greater_than.eq(_i);
				var _form_value = parseInt(_currentForm.data("ifValue"));
				if (_form_value <= value) {
					_currentForm.removeClass("hidden");
				}
			}
		});
	...
	function initPreview() {
		var myJson = localstorage.get("adminInputs");
		...
		var preview = $("#preview");
		preview.html("");
		adminInputs.forEach(function (input, index) {
	...
	function addUserSubInputs(input, x, level) {
		...
		if (input.subinputs && input.subinputs.length > 0 || x < 50) {
			level++;
			input.subinputs.forEach(function (subinput, index) {
	...
	function addUserInput(input, index) {
		var level = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 0;

		var preview = $("#preview");
		var show = true;
		var form = $("<form></form>").addClass("input-box");
		var div = $("<div></div>").addClass("form-group");

		input.formGroups.forEach(function (formGroup) {
			if (formGroup.name) {
				var name = formGroup.name.toLowerCase();
				if (name === "condition") // czyli to jest subinput
					{
						var options = formGroup.types[0].options;
						var finded = (0, _lodash.findIndex)(options, { default: true });
						if (finded >= 0) {
				...
				if (name === "question") {
					var label = $("<label></label>").addClass("control-label").text(formGroup.value);
					div.append(label);
				} else if (name === "type") {
					formGroup.options.forEach(function (option) {
						if (option.default) {
							if (option.value === "text") {
								var formInput = $("<div></div>").append($("<input>").addClass("form-control").attr("type", "text"));
								div.append(formInput);
							} else if (option.value === "number") {
								var _formInput = $("<div></div>").append($("<input>").addClass("form-control").attr("type", "number"));
								div.append(_formInput);
							} else if (option.value === "yes_no") {
								var _formInput2 = $("<div></div>").append($("<input>").attr("type", "radio").attr("name", "radio-" + index).attr("id", "radio-" + index + "-yes").val(1)).append($("<label></lable>").text("Yes").attr("for", "radio-" + index + "-yes")).append($("<input>").attr("type", "radio").attr("name", "radio-" + index).attr("id", "radio-" + index + "-no").val(0)).append($("<label></lable>").text("No").attr("for", "radio-" + index + "-no"));
								div.append(_formInput2);
							}
						}
					});
				}
	...
	function search(value) {
		var mainIndex = (0, _lodash.findIndex)(adminInputs, { 'id': value });

		if (mainIndex === -1) {
			for (var i = 0, len = adminInputs.length; i < len; i++) {
	...
	function wglab(current, value, address) {
		var parentprev = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : true;
		...
		while (current.subinputs && current.subinputs.length > 0 || x < 50) {
			...
			if (parent.subinputs && parent.subinputs.length > 1) {
				for (var i = 1; i < parent.subinputs.length; i++) {
					myArray.push(i);
	...
	function addInputButton() {
		...
		var inputBox = defaultSchemeInputs.inputBox;
	...
	function addInput(args) {
		var userInputScheme = mixins.getUserInputScheme(),
		...
		inputBox.myClass.forEach(function (inputClass) {
			myClass += inputClass + " ";
		});
		box.addClass(myClass);

		inputBox.formGroups.forEach(function (formGroup, index) {
			var type = "",
			    subtype = "",
			    label = "",
			    idName = "";
			...
			if (formGroup.type) {
				switch (formGroup.type) {
					case "text":
						...
						type = $("<input>").attr("type", formGroup.type).addClass("form-control").attr("id", idName + "-" + len + "-" + index).attr("type", "text");
						if (!!formGroup.value) {
							type.val(formGroup.value);
						}
						label = $("<label></label>").addClass("col-xs-2 control-label").attr("for", idName + "-" + len + "-" + index).text(formGroup.name + ":");
						break;
					case "select":
						idName = mixins.convertToSlug(formGroup.name);

						type = $("<select></select>").attr("id", idName + "-" + len + "-" + index).addClass("form-control subif-select");

						formGroup.options.forEach(function (option) {
							var optionTag = $("<option></option>");

							optionTag.val(option.value).text(option.text);
					...
					case "buttons":
						type = [];
						formGroup.buttons.forEach(function (formbutton) {
							var button = $("<button></button");

							myClass = "";
							formbutton.myClass.forEach(function (buttonClass) {
								myClass += buttonClass + " ";
							});
				...
				types = [];
				formGroup.types.forEach(function (el, lindex) {
					switch (el.type) {
						case "select":
							idName = mixins.convertToSlug(formGroup.name);

							subtype = $("<select></select>").attr("id", idName + "-" + len + "-" + index + "-" + lindex).attr("data-if", el.if).addClass("form-control condition").css({
								"width": "50%",
								"float": "left"
							});
							...
							el.options.forEach(function (option) {
								...
								optionTag.val(option.value).attr("data-if", option.if).attr("data-show", 0).text(option.text);
							...
							types.push(subtype);
							label = $("<label></label>").addClass("col-xs-2 control-label").attr("for", idName + "-" + len + "-" + index + "-" + lindex).text(formGroup.name + ":");
							break;
						case "text":
							idName = mixins.convertToSlug(formGroup.name);

							subtype = $("<input>").attr("type", formGroup.type).addClass("form-control").attr("data-if", el.if).attr("id", idName + "-" + len + "-" + index).css({
								"width": "50%",
								"float": "left"
							});
			...
			var innerDiv = $("<div></div>").addClass(myClass);

			if (formGroup.types) {
				types.forEach(function (tmp) {
					innerDiv.append(tmp);
				});
			...
			var div = $("<div></div>").addClass("form-group").attr("data-from-group-element", index).append(label).append(innerDiv);

			box.append(div);
		});

		target.append($("<section></section>").addClass("input-" + inputBox.id).attr("data-level", level).attr("data-id", inputBox.id).css({
	...
	function formGroupChange(el) {
		...
		wto = setTimeout(function () {
			var formGroupAdmin_id = parseInt(el.closest(".form-group").data("fromGroupElement")),
			...
			if (el.is("input")) {
				...
				if (tmpadmins.formGroups[formGroupAdmin_id].options) {
					tmpadmins.formGroups[formGroupAdmin_id].options.forEach(function (option) {
						option.default = option.value === el.val() ? true : false;
					});
				} else if (el.val() == 0 || el.val() == 1) {
					tmpadmins.formGroups[formGroupAdmin_id].types[1].options.forEach(function (option) {
						option.default = option.value == el.val() ? true : false;
					});
					tmpadmins.formGroups[formGroupAdmin_id].value = parseInt(el.val());
				} else {
					tmpadmins.formGroups[formGroupAdmin_id].types[0].options.forEach(function (option) {
						option.default = option.value === el.val() ? true : false;
					});
				}

				tmpadmins.subinputs.forEach(function (subinput) {

					var formGroup = subinput.formGroups[0];
					if (formGroup.name === "Condition") {
						formGroup.types[0].options.forEach(function (option) {
...
exports.getDefaultSchemeInputs = getDefaultSchemeInputs;
exports.getUserInputScheme = getUserInputScheme;
